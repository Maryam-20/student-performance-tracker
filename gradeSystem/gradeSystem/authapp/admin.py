...
class AllUserAdmin(admin.ModelAdmin):
    list_display = ('email', 'is_staff', 'is_teacher', 'is_verified', 'is_active')
    search_fields = ('first_name', 'last_name', 'email')
    list_filter = ('is_staff', 'is_active' , 'is_verified', 'is_teacher')
    ordering = ('-date_joined',)
    list_per_page = 20

@admin.register(Student)
class StudentAdmin(admin.ModelAdmin):
    list_display = ('id', 'full_name', 'gender', 'grade_level', 'class_name', 'phone_number', 'address', 'date_of_birth')
    search_fields = ('id', 'full_name', 'class_name')
    list_filter = ('grade_level', 'class_name')
    ordering = ('-date_updated',)


@admin.register(Staff)
class StaffAdmin(admin.ModelAdmin):
    list_display = ('id', 'full_name', 'gender', 'phone_number', 'position', 'address', 'date_of_birth')
